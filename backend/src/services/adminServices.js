...
async function checkAdminId(id) {
  try {
    ...
    const analysis = await knex("administrador")
      .select("*")
      .where({ id })
      .first();
...
async function viewAdmin(idAdmin) {
  try {
    ...
    const adminInfo = await knex("administrador")
      .select("*")
      .where({ id })
      .first();
    if (!adminInfo) {
...
async function createAdmin({ nome, cargo, cpf, telefone, senha }) {
  try {
    const cpfExistente = await knex("administrador")
      .select("*")
      .where({ cpf })
      .first();
    if (cpfExistente) {
...
async function updateAdmin(idAdmin, updateData) {
  try {
    const id = idAdmin;
    const idExiste = await knex("administrador").where({ id }).first();
    ...
    await knex("administrador").where({ id }).update(fieldsToUpdate);
...
async function deleteAdmin(idAdmin) {
  try {
    const id = idAdmin;
    const adminExistente = await knex("administrador")
      .select("*")
      .where({ id });
