...
async function updateMetodo_Pagamento(req, res) {
    try {
        ...
        const metodo_pagamento_atualizado = await metodo_pagamentoService.findOneMetodo_Pagamento()
        res.status(200).json({ status: true, message: "Método de pagamento atualizado com sucesso!", metodo_pagamento_atualizado});
    } catch (erro) {
        console.error("Erro no controller:", erro);
