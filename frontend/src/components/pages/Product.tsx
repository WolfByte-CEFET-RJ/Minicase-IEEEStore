...
export default function Product() {
    ...
    const [productData, setProductData] = useState({nome: "a", preco:0, quantidade:1})
    ...
    useEffect(() => {
        ...
        const fetchProduto = async () => {
            try {
                ...
                setProductData(response.data.produto);
            } catch (error) {
        ...
        <section className="p-5 mt-10 shadow-md">
            <div className="flex flex-row items-center">
                ...
                <div className="flex flex-col gap-6 border-l pl-10">
                    <h2 className="font-bold text-3xl">{productData.nome}</h2>
                    <p className="text-xl">R$ {productData.preco}</p>
                    <div className="flex flex-row items-center border border-blue-800 justify-between px-2 py-1 rounded-xl w-24 text-xl">
                        ...
                        <p>{quantity}</p>
                        <button className="font-bold" onClick={() => setQuantity((quantity) => Math.min(quantity + 1, productData?.quantidade))}>+</button>
                    </div>
